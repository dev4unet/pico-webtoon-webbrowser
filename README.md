...
개인적으로 Pico4 Ultra VR환경에서는 게임보다는 `주로 방에 누워서 자체 제공되는 VR용 기본 웹 브라우저를 이용`해서 편하게 제 블로그 글을 확인하거나 가끔씩 네이버 웹 툰 등의 웹 기반 서핑을 하는데, `제가 시력이 안 좋아서 안경 없이 화면을 크게 확대해서 사용`하는 데 자체 제공되는 VR용 기본 웹브라우저는 가로나 세로로 편하게 사이즈 조절이 가능해서 큰 불편 없이 사용하고 있었는데 `세로 길이가 조금 더 길어졌으면 하는 아쉬움`이 생겼습니다.
...
PICO 자체의 기본 웹 브라우저로 제 블로그를 방문했을 때와 이번에 만든 피코 웹툰 웹 브라우저로 방문했을 때의 기본 실행 화면입니다.   

![앱 스크린샷](docs/sshot1-500.jpeg)

위 사진은 웹툰 웹 브라우저의 세로 모드로 실행했을 때의 기본 화면 사이즈를 기준으로 PICO 자체의 기본 웹 브라우저로 높이만 맞췄습니다. (PICO 자체 웹 브라우저는 가로/세로 크기가 자유롭게 수정 가능해서 애용하고 있습니다.)

하지만, PICO 자체 웹 브라우저는 늘릴 수 있는 세로 높이 제한이 있으며, 이번에 만든 피코 웹툰 웹 브라우저는 대각선으로만 길이가 늘어나지만 PICO 자체 웹 브라우저 보다 더 높게 늘어나서 개인적으로는 좋네요.(다만, 화면에 방탄형은 아니라서 다른 분들에게는 어떨지 모르겠네요.)

![앱 스크린샷](docs/sshot2-500.jpeg)


지금 생각해 보니 웹 툰전용 웹 브라우저를 찾아서 설치하는 것이 더 좋았을 것 같기도 합니다.^^

개인적으로 포스팅했던 `알리익스프레스 관련 최신 할인 정보 글`도 검토하고 제 블로그 방문이 대부분이라 `시작 주소를 제 블로그 글`로 해 놓았지만, `시작 주소를 변경하고 싶은 분들을 위해 홈페이지 설정 기능`도 제공되며 소스도 공개로 제공되니 각자 편한대로 사용하세요.

저한테는 필요 없지만 반응형처럼 동작은 안해서 `PC모드에서 줌 기능`을 설정할 수 있도록 기능 추가 했습니다.
...
```
- `screenOrientation="portrait"` — 기본 세로 모드 (PC 모드 전환 시 런타임에서 landscape로 변경)
- `resizeableActivity="true"` — PICO VR에서 대각선 리사이즈 허용
- PICO VR에서 세로형 2D 플로팅 윈도우로 실행되며, 대각선으로 크기 조절 가능
...
### Debug APK 빌드
```batch
cd C:\workspace\pico-webtoon-webbrowser
set JAVA_HOME=C:\Program Files\Microsoft\jdk-17.0.17.10-hotspot
set ANDROID_HOME=C:\Users\사용자명\AppData\Local\Android\Sdk
